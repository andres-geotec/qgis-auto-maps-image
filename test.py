...
def formatLegendlabel(eval, i):
    ...
    if i == 0: return f'{format(min)} - {format(max)} ({countFeaturesByQuery(query, muns):,})'
    else: return f'{format(str(float(min)+0.1))} - {format(max)} ({countFeaturesByQuery(query, muns):,})'
...
def settingsLegend(legend, legendName):
    ...
    for i, node in enumerate(legend.model().layerLegendNodes(legendlayer)):
        node.setUserLabel((formatLegendlabel(node.evaluateLabel(), i)))
